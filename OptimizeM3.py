from custom.util.data_manipulation import load_and_process_data, split_data
...
data = load_and_process_data("data/claims_train.csv")
X_train, X_test, y_train, y_test = split_data(
    numeric_features = ["VehPower", "VehAge","BonusMalus", "Density", "Interaction1",  "DrivAge"],
    categorical_features = ["VehGas"],
    target = "Risk",
    df = data,
    test_size=0.2,
...
fine_grid_rf = generate_fine_grid(rs_rf.best_params_, spread=0.1, num_steps=2)
fine_grid_gb = generate_fine_grid(rs_gb.best_params_, spread=0.1, num_steps=2)
fine_grid_mlp = generate_fine_grid(rs_mlp.best_params_, spread=0.2, num_steps=2)

print("New RF Grid:", fine_grid_rf)

print("Running Fine-Tuning GridSearch for RF...")
gs_rf = GridSearchCV(RandomForestRegressor(random_state = random_state), fine_grid_rf, cv=5, n_jobs=-1, verbose=2)
gs_rf.fit(X_train, y_train)

print("Running Fine-Tuning GridSearch for GB...")
gs_gb = GridSearchCV(GradientBoostingRegressor(random_state = random_state), fine_grid_gb, cv=5, n_jobs=-1, verbose=2)
gs_gb.fit(X_train, y_train)

# For MLP (Pipeline)
gs_mlp = GridSearchCV(mlp_pipe, fine_grid_mlp, cv=5, n_jobs=-1, verbose=2)
