All data columns were used as features for the preddictions with \texttt{BonusMalus}, \texttt{Density}, \texttt{DrivAge}, and \texttt{VehAge} were treated as continous numeric featuress with the rest treated as categorical and one-hot encoded. The columns on which \texttt{Risk} calculations are based are \texttt{ClaimNb} and \texttt{Exposure}, therefore not included in the feature set.

\subsection{Claims Risk}
Claims Risk were computedd using the Bayesian Credibility using the Bühlmann Linear Approximation \cite{Bühlmann_1969,buhlmann1970glaubwurdigkeit} with the assumption that data points with low exposesure lacked credibility to say that the driver is more risky at a certain claims per exposure ratio, since luck can be big factor in a driver getting into an accident within one day, however over a longer period of time it is assumed that the driver has enogh time to show their true risk.
...
$$
So for data points with low credibility the average claim to exposure ratio dominates the risk calculation and oppositely data points with high credibility are primarely assigned their claim to exposure ratio, while the data points in the middle are mixed depending on their credibility. However, because of the number of claims being heavily biased toward zero the risk score is also very imbalanced~\autoref{fig:distribution-risk}
