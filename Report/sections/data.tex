...
\begin{enumerate}
    \item[a.] \texttt{IDpol} - as it is not a feature
    \item[b.] \texttt{Area} - categorical counterpart of \texttt{Density}, therefore not needed
    \item[c.] \texttt{VehBrand} - Vehicle brand alone is not an indicator of policy risk, especially since a singular brand has many models of vehicle, potentially attracting different driver types, however, we assume that the risk evens out along the whole brand's lineup of cars.
    \item[d.] \texttt{Region} - another indicator of \texttt{Density}. 
\end{enumerate}
